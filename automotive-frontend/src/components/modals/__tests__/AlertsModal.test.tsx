...
describe('AlertsModal', () => {
  ...
  it('should display empty state when vehicle has no alerts', () => {
    render(<AlertsModal isOpen={true} onClose={() => {}} vehicle={mockVehicleWithoutAlerts} />);

    expect(
      screen.getByText(/No hay alertas de mantenimiento para este vehículo/)
    ).toBeInTheDocument();
  });
  ...
  it('should call onClose when close button is clicked', async () => {
    ...
    render(<AlertsModal isOpen={true} onClose={mockOnClose} vehicle={mockVehicleWithAlerts} />);
  ...
  it('should not render when modal is closed', () => {
    const { container } = render(
      <AlertsModal isOpen={false} onClose={() => {}} vehicle={mockVehicleWithAlerts} />
    );
  ...
  it('should not render when vehicle is null', () => {
    const { container } = render(<AlertsModal isOpen={true} onClose={() => {}} vehicle={null} />);
  ...
  it('should apply correct CSS classes for alert types', () => {
    const { container } = render(
      <AlertsModal isOpen={true} onClose={() => {}} vehicle={mockVehicleWithAlerts} />
    );
