...
from src.infrastructure.database.models import Base, VehicleModel
...
def test_status_field_integration():
    ...
    try:
        ...
        result = session.execute("PRAGMA table_info(vehicles)")
        columns = {row[1]: row[2] for row in result}
        assert "status" in columns
        print(f"   ✓ Status column exists in database")
